...
import com.ocean.driver.dialog.BindNewPhoneSuccessDialog;
...
public class BindNewPhoneGetPhoneCodeActivity extends BaseActivity {
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.btn_next:
                ...
                HttpUtil.createRequest(TAG, BaseUrl.getInstence().changePhone()).changePhone(PreferenceUtils.getInstance().getUserToken(), phoneNum, etPhoneCode.getText().toString()).enqueue(new Callback<ApiResponse>() {
                    @Override
    ...
    private void getPhoneCode(){
        HttpUtil.createRequest(TAG, BaseUrl.getInstence().sendSmsNew()).sendSmsNew(PreferenceUtils.getInstance().getUserToken(), phoneNum).enqueue(new Callback<ApiResponse>() {
            @Override
            public void onResponse(Call<ApiResponse> call, Response<ApiResponse> response) {
                if (response.body() != null) {
                    if (response.body().getCode() == 1) {
