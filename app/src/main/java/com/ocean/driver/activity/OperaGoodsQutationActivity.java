...
public class OperaGoodsQutationActivity extends BaseActivity {
    ...
    public static void actionStart(Context context, String o_id) {
    ...
    protected void initDatas() {
        HttpUtil.createRequest(TAG, BaseUrl.getInstence().operationListingGoodsList()).operationListingGoodsList(PreferenceUtils.getInstance().getUserToken(), getIntent().getStringExtra(O_ID)).enqueue(new Callback<ApiResponse<OperaGoodsListData>>() {
            @Override
            public void onResponse(Call<ApiResponse<OperaGoodsListData>> call, Response<ApiResponse<OperaGoodsListData>> response) {
                if (response.body().getCode() == 1) {
                    ...
                    tvNum.setText("数量："+response.body().getData().getTotal_info().getGoods_num());
                } else {
