...
import com.ocean.driver.adapter.LoadingTypeTwoAdapter;
...
public class LoadingTypeTwoFragment extends BaseFragment {

    @BindView(R.id.rv_loading_type)
    RecyclerView rvLoadingType;
    @BindView(R.id.sv_list)
    SpringView svList;
    private LoadingTypeTwoAdapter adapter;
    ...
    protected void initViews() {
        initSpringViewStyle();
        adapter = new LoadingTypeTwoAdapter(getActivity());
    }
    ...
    private void getData() {
        HttpUtil.createRequest(BaseUrl.getInstence().listingGet()).listingGet(PreferenceUtils.getInstance().getUserToken(), page + "", 6 + "").enqueue(new Callback<BillData>() {
            @Override
