...
public class OperaListFragment extends BaseFragment {
    ...
    @SuppressLint("ValidFragment")
    public OperaListFragment(String type) {
        this.type = type;
    }
...
//                        Log.e("经纬度",aMapLocation.getLatitude()+"---"+aMapLocation.getLongitude()+"---城市："+aMapLocation.getCity());
    ...
    private void getData() {
        HttpUtil.createRequest(BaseUrl.getInstence().operationListingGet()).operationListingGet(PreferenceUtils.getInstance().getUserToken(), page + "", type + "").enqueue(new Callback<OperaListData>() {
            @Override
