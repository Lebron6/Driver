...
public class LoadingTypeOneFragment extends BaseFragment {
    ...
    protected void initViews() {
        initSpringViewStyle();
        adapter = new LoadingTypeOneAdapter(getActivity());
    }
    ...
    private void getData() {
        HttpUtil.createRequest(BaseUrl.getInstence().listingGet()).listingGet(PreferenceUtils.getInstance().getUserToken(), page + "", 5 + "").enqueue(new Callback<BillData>() {
            @Override
